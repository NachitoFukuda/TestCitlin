...
import { sampleQuestions, passageText } from '../../constants/2/questions2';
...
const ExamQuestionCard: React.FC = () => {
  ...
  const [isImageVisible, setIsImageVisible] = useState(false); // 画像表示の状態を管理
  ...
  if (examFinished) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.header}>２級評価結果</Text>
        <ScrollView
  ...
  if (showCard) {
    return (
      ...
      {!isImageVisible && ( // 状態が true のときのみ画像を表示
      ...
      <TouchableOpacity
        style={styles.button}
        onPress={() => setIsImageVisible(!isImageVisible)} // ボタン押下で表示切替
      >
        <Text style={styles.buttonText}>
          {isImageVisible ? 'テキストを表示するする' : '画像を表示する'}
        </Text>
      </TouchableOpacity>
    </NeomorphBox>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.questionContainer}>
        ...
        */}
        ...
        {(questionNumber === 2 ) && (
          <NeomorphBox width={320} height={600} forceTheme={'light'}>
            <Text style={styles.passageTitle}>card</Text>
            <Image
              source={require('../../assets/images/2.jpg')}
              style={styles.passageImage}
            />
                <Text style={styles.passageText}>
                  One day, Yamada discovered a relaxation facility at the station.
                </Text>
